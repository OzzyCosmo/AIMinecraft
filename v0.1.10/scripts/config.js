...
export const CLOUD_FRAGMENT_SHADER = `
  ...
  float calcShadow(vec3 pos, vec3 lightDir, float coverageThreshold, float densityStrength, vec2 bounds, vec2 windOffset) {
    ...
    vec3 samplePos = pos + lightDir * (stepLen * (0.35 + jitter * 0.65));
  ...
  void main() {
    ...
    float steps = clamp(quality, 16.0, float(MAX_STEPS));
    float stepLength = max(1.2, (end - start) / steps);
    float searchStep = stepLength * 3.5;
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
      ...
      if (probeDensity > 0.015) {
        t = max(start, coarseT - stepLength * 1.5);
        break;
      }
      coarseT += searchStep;
      t = coarseT;
